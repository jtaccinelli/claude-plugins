...
For each entry cell, spawn a sub-agent using the **cell-agent** agent with `--cell <concern.layer>`, instructing it to determine what the request needs from *this* cell and, for anything this cell cannot itself provide, author a contract for what it needs from another cell (per its Assessing Contracts / Creating Items actions and the request description).

For each authored contract:

1. As **coordinator**, run [Routing Contracts](../../agents/coordinator/actions/routing-contracts.md) — classify the target cell if not already named, and hand the contract to that cell's cell-agent.
2. That cell-agent runs [Assessing Contracts](../../agents/cell-agent/actions/assessing-contracts.md), returning Met / Partial · additive / Partial · breaking / Unmet / Countered.
...
As **coordinator**, run [Merging Duplicates](../../agents/coordinator/actions/merging-duplicates.md) whenever two contract chains reach the same provider item, or two contracts share a canonical key (target cell + ratified output-shape signature). Do this continuously as Step 2 proceeds, not as an afterthought — it keeps the historical-demand counters in every cell's `REQUESTS.md` trustworthy.
