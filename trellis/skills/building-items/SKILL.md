...
Run an approved plan to completion. For each build wave, every ready item is built by its owning node-agent inside its own worktree, reviewed by the coordinator for contract fidelity, retried on failure up to a cap, and self-documented on pass. When every item across every wave has landed (or is blocked), the coordinator wraps the request inline: `SUMMARY.md` written, PR opened — without returning control to the user between waves.
...
2. Read `PLAN.md` in full — entry nodes, contract graph, Met/Modify/Create breakdown, proposed build waves.
...
- Derive the attempt branch: `attempt/<slug>-<node>-<item-slug>-<NNN>` (NNN = zero-padded attempt count).
- Derive the worktree path: `.worktrees/attempt-<slug>-<node>-<item-slug>-<NNN>` (relative to repo root).
- Create the worktree from the request branch: `git worktree add -b attempt/<slug>-<node>-<item-slug>-<NNN> .worktrees/attempt-<slug>-<node>-<item-slug>-<NNN> request/<slug>`.
...
- **Node Agent Summary** — factual account of what was built.
...
1. Force-remove the worktree: `git worktree remove --force .worktrees/attempt-<slug>-<node>-<item-slug>-<NNN>`. Delete the attempt branch: `git branch -D attempt/<slug>-<node>-<item-slug>-<NNN>`.
...
- **Description** — 2–4 plain-language sentences: what capability was added, grouped by node.
- **Items** — every item built, grouped by node, with a one-line description each (draw from each item's `USAGE.md`).
...
- `.claude/trellis/<node>/<item>/attempts/ATTEMPT_<NNN>.md` — attempt file per attempt executed
- `.claude/trellis/<node>/<item>/{CONTRACT,USAGE,CHANGELOG,REFERENCES}.md` — updated on pass
- `.claude/trellis/<node>/INVENTORY.md` — regenerated on pass
- `.claude/trellis/<node>/AGENT.md` — Learned Patterns log appended on pass, only when a new pattern is confirmed
...
- Never dispatch a node-agent without `--node`.
- Never skip Reviewing Fidelity — an attempt is never marked passed without it.
- Never infer the changed file list from node-agent output — always derive from `git status --short` in the worktree.
...
- If a previous attempt exists for an item, always pass its attempt file path to the node agent — never run a subsequent attempt without it having access to prior Fidelity Notes.
