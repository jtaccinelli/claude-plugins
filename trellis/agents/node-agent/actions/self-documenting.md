...
4. Regenerate this item's `REFERENCES.md` and this node's `INVENTORY.md` by following the `Source` pointer out to the real code and scanning its imports — both are generated, never hand-maintained, so a full regeneration each time is correct, not wasteful.
