...
description: Record what was just built, regenerate the item's and node's derived files, and log any new pattern into the node's AGENT.md.
---
...
**Inputs**: what was just built, the coordinator's Reviewing Fidelity notes (including any pattern findings), this node's `AGENT.md` Learned Patterns section.
...
4. Regenerate this item's `REFERENCES.md` and this node's `INVENTORY.md` by following the `Source` pointer out to the real code and scanning its imports — both are generated, never hand-maintained, so a full regeneration each time is correct, not wasteful. If this is the item's first pass, create `REFERENCES.md` from `trellis/skills/building-items/templates/REFERENCES.md`. Format: `Used by 0 items.` when the scan finds none, or `Used by <N> items:` followed by one bullet per consumer (`` `<concern> · <layer>` → `<item-slug>` ``) when it finds any — never hand-write this list, always derive it from the scan.
...
**Report** (returned to the dispatcher): updated item leaf (`CONTRACT.md`, `USAGE.md`, `CHANGELOG.md`, `REFERENCES.md`), this node's `INVENTORY.md`, and — when a new pattern was logged or a conflict was raised — this node's `AGENT.md`.
