...
The writer receives a requirement (feature description, bug report, or change request) and produces a `manuscript.md`. The writer self-validates the cast against available roles and backgrounds before finalising. Once confirmed, the manuscript is handed to the director, who uses it to govern the scene run.

If a take produces findings that the manuscript was unclear or incorrect, the writer revises it — applying corrections to the affected sections only, not rewriting the manuscript wholesale.
